...
Roadmap::Roadmap()
{
  ...
  // Subscribers
  waypoints_sub_ = nh_.subscribe(config_->external_waypoint_topic_, 1, &Roadmap::WaypointCallback,
                                 this);  // Subscriber for waypoints (forwarded to the reader)

  // Debug: listens to rviz point for translating the map
  offset_sub_ = nh_.subscribe("roadmap/offset", 1, &Roadmap::OffsetCallback,
                              this);  // Subscriber for waypoints (forwarded to the reader)
