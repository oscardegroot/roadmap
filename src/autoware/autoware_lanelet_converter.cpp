    ...
    : rclcpp::Node("lanelets_to_path")
{
    ...
    vector_map_subscriber_ = roadmap->create_subscription<HADMapBin>(
        ...
        createSubscriptionOptions(roadmap_ptr_.get()));
    ...
    autoware_path_publisher_ = roadmap->create_publisher<PathWithLaneId>(
...
void AutowareLaneletConverter::onOdometry(Odometry::ConstSharedPtr msg)
{
    ...
    else
    {
        ...
        {
            if (config_ptr_->debug_output_)
                LOG_INFO(logger_, "Updating odometry and route");
...
{
    if (config_ptr_->debug_output_)
        LOG_INFO(logger_, "Route Received");
    ...
    LOG_INFO("Computing new lanelet route to the goal");
...
{
    LOG_INFO("Received " << std::floor(msg.poses.size()) << " Waypoints");
    ...
    new_way.AddLane("road", 4.0, true, 0); // Add a lane
    auto &lane = new_way.lanes().back();
    FitSplineOnLane(lane); // Fit a spline on the received waypoints
    ...
    reference_pub_->publish(msg); // publish
}